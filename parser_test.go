...
func (s *testParserSuite) TestQuotedSystemVariables(c *C) {
	...
	st, err := parser.ParseOneStmt("select @@`SQL_MODE`, @@session.`sql_mode`, @@global.`s ql``mode`;", "", "")
	c.Assert(err, IsNil)
	...
	for i, field := range ss.Fields.Fields {
		...
		c.Assert(ve.Name, Equals, expected[i].Name)
		c.Assert(ve.IsGlobal, Equals, expected[i].IsGlobal)
		c.Assert(ve.IsSystem, Equals, expected[i].IsSystem)
		c.Assert(ve.ExplicitScope, Equals, expected[i].ExplicitScope)
	}
}
